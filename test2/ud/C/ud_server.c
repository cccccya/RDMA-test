...
static int poll_completion(struct pingpong_context *res, int *pos, int *number) {
    ...
    if(poll_result < 0) {
        ...
        for(int i = 0;i < poll_result; i++){
            ...
            if(wc[i].opcode & IBV_WC_RECV) {
                ...
                /*post_recv(res, (*pos)*140);
                (*pos)=((*pos)+1)%1024;*/
                if(((*number)+i+1)%64==0){
                    char tmp[66536];memset(tmp,0,sizeof(tmp));
                    for(int i=0;i<64;i++){
                        memcpy(tmp,res->buf+(*pos)*1064+40,1024);
                        post_recv(res, (*pos)*1064);
                        (*pos)=(*pos+1)%40;
                    }
                    //printf("%s\n",tmp);
                }
...
int main(int argc, char **argv)
{
    ...
    int sum = 0, pos = 0, tot_num = 1ll*1024*1024*100*100/1024,number = 0;;
    while(sum < tot_num) {
        int cnt = 0;
        cnt = poll_completion(ctx, &pos, &number);
        if(cnt <= 0) {
            printf("%d %d\n",sum, tot_num);
            printf("轮询失败\n");
            exit(0);
